...
my %TypeStr = (Video::Capture::ZVBI::VBI_OPTION_BOOL => "BOOL",
               Video::Capture::ZVBI::VBI_OPTION_INT => "INT",
               Video::Capture::ZVBI::VBI_OPTION_REAL => "REAL",
               Video::Capture::ZVBI::VBI_OPTION_STRING => "STRING",
               Video::Capture::ZVBI::VBI_OPTION_MENU => "MENU");
...
sub test_set_int {
	...
	if (!$ex->option_get($oi->{keyword}, &new_current)) {
...
sub test_set_entry {
	...
	my $new_current = $ex->option_get($oi->{keyword});
        die $ex->errstr() unless defined $new_current;
	if ($r0 == 0 || $r0 == 1) {
		test_modified($oi, $$current, $new_current);
        }
	...
	my $new_entry = 0x33333;
	my $r1 = $ex->option_menu_get($oi->{keyword}, &new_entry);
...
sub dump_option_info {
	...
	$type_str = $TypeStr{$oi->{type}};
        ...
        $oi->{keyword} = "" unless defined $oi->{keyword};
        $oi->{label} = "" unless defined $oi->{label};
        $oi->{tooltip} = "" unless defined $oi->{tooltip};
        $oi->{mime_type} = "" unless defined $oi->{mime_type};
        $oi->{extension} = "" unless defined $oi->{extension};
	...
	if (($oi->{type} == Video::Capture::ZVBI::VBI_OPTION_BOOL) ||
		...
		$val = $ex->option_get($oi->{keyword}, &val);
                die unless defined $val;
		print_current($oi, $val);
		if ($check) {
			if ($oi->{menu}) {
		...
		if ($check) {
			...
			die unless defined $ex->option_get($oi->{keyword}, &val);
			printf("    current value=\"%s\"\n", $val);
		}
...
sub list_options {
	...
	print "  List of options:\n";
		...
		die unless defined $ex->option_info_keyword($oi->{keyword});
...
sub list_modules {
	...
	print "List of export modules:\n";
		...
		$xi->{keyword} = "" unless defined $xi->{keyword};
                $xi->{label} = "" unless defined $xi->{label};
		$xi->{tooltip} = "" unless defined $xi->{tooltip};
                $xi->{mime_type} = "" unless defined $xi->{mime_type};
                $xi->{extension} = "" unless defined $xi->{extension};
		...
		my $ex = Video::Capture::ZVBI::export::new($xi->{keyword}, $errstr);
                die "Could not open $xi->{keyword}\n" unless defined $ex;

		die unless defined $ex->info_export();
