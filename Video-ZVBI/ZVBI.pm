...
You can do so later with I<$cap-E<gt>update_services()> (Note the I<reset>
...
client context (i.e. class B<Video::Capture::ZVBI::proxy>.) The remaining
parameters have the same meaning as described above, as they are used
when opening the device on server side.
...
decodable services.  See I<Video::Capture::ZVBI::raw_dec::add_services>
...
an previously opened capture context (which is implicitly inserted as
...
within B<$timeout_ms> milliseconds) or 1 on success.
...
copy_sliced_line() to process the contents in Perl, or pass the buffer
directly to the VT or other decoder objects.
...
I<$ref> in form of a blessed reference to an internal buffer. The data
remains valid until the next call to this or any other "pull" function.
The reference can be passed to copy_sliced_line() to process the data in
...
I<Video::Capture::ZVBI::capture::parameters>.)
...
Removes one or more data services to be decoded from the raw decoder
context.  This function can be called at any time and does not touch
sampling parameters stored in the context. 
...
The input can either be a scalar filled by one of the "read" kind of
capture functions (or a scaler filles with a byte string with the correct
...
for I<read_sliced()>.  Return value is the number of lines decoded.
...
raw decoder context for different devices.

=back

=head1 Video::Capture::ZVBI::vt

This section described high level decoding functions.  Input the
decoder function in this section is sliced data, as returned from
...
callback using I<$vt-E<gt>event_handler_add()>.
...
and must be exactly the value returned by the slicer function.
...
Optional parameter I<$nuid> is currently unused by libzvbi and dfaults
to zero.
...
news updates, program related pages) subpage numbers can increase or
page types and languages can change.
...
page object of type C<Video::Capture::ZVBI::page>.  The reference can
...
to the libzvbi function I<vbi_unref_page> as it's not needed.
...
of type C<Video::Capture::ZVBI::page>.
...
With I<$vt-E<gt>event_handler_add()> you can request a C<VBI_EVENT_CAPTION>
...
this function again to update other displays.
...
=item $pg->draw_vt_page_region($fmt, $canvas, $rowstride, $col_off, $row_off, $column, $row, $width, $height, $reveal=0, $flash_on=0)
...
The image is returned in a scalar which contains a byte string.  Each
pixel uses 4 subsequent bytes in the string (RGBA). Hence the string
is at least C<$rowstride * 10 * ($row_off + $height)> bytes long. 
...
I<$column> is the first source column (range 0 ... pg->columns - 1);
I<$row> is the first source row (range 0 ... pg->rows - 1);
I<$width> is the number of columns to draw, 1 ... pg->columns;
I<$height> is the number of rows to draw, 1 ... pg->rows;
...
=item $xpm = $pg->rgba_to_xpm($canvas, $rowstride=-1, $width=0)
...
calculated from the given I<$width> parameter, which is otherwise
unused.
...
but guarantees that characters in the returned string correlate
exactly with the array returned by I<$pg-E<gt>get_page_text_properties()>.

When the optional parameter I<$all_chars> is set to 1, even
characters on the private Unicode code pages are returned.
...
type, keyword, label, def, min, max, step, menu, tooltip.
...
=item $search = vbi_search_new($vt, $pgno, $subno, $pattern, $casefold=0, $regexp=0, $progress=NULL)
...
The script captures from C</dev/vbi0> until the currently tunes channel is
